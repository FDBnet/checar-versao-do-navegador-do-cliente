 ...
 *     Hints), Vivaldi, Yandex, UC Browser, Whale, DuckDuckGo, Amazon Silk, Maxthon,
 *     QQ Browser, Baidu, 360 e Internet Explorer 11. Sem colisões silenciosas.
 ...
 *
 * Novidades da 3.3.0:
 *   - Mais navegadores: Amazon Silk, Maxthon, QQ Browser, Baidu e 360. QQ/Baidu/360 são
 *     classificados pela versão do Chrome embutido (a versão própria não reflete a
 *     capacidade) com limiares permissivos para não incomodar motores embutidos antigos.
 *
 * Novidades da 3.2.0:
...
(function (root, factory) {
     ...
     * @property {string|null} codigo             Código curto ('c','f','s','o','e','i','sm','b','a','v','y','uc','ddg','w','sl','mx','qq','bd','360') ou null.
    ...
    var VERSOES_PADRAO = {
        ...
        w:    [3,    3],        // Whale (Naver)
        sl:   [90,   100],      // Amazon Silk (versão acompanha o Chromium)
        mx:   [6,    7],        // Maxthon (versão própria)
        // QQ, Baidu e 360 são classificados pela versão do Chrome EMBUTIDO (a versão
        // própria não reflete a capacidade real). Limiares permissivos de propósito:
        // muitos rodam motores X5/embutidos antigos que o usuário não pode atualizar.
        qq:   [70,   90],       // QQ Browser / motor X5 (ex.: navegador interno do WeChat)
        bd:   [70,   90],       // Baidu (baiduboxapp / BaiduBrowser)
        '360': [70,  90]        // 360 Secure/Speed Browser (não versiona a si mesmo no UA)
    };
    ...
    var URLS_PADRAO = {
        ...
        sl:   'https://www.amazon.com/',          // Silk se atualiza pelo Fire OS/Appstore
        mx:   'https://www.maxthon.com/',
        qq:   'https://browser.qq.com/',
        bd:   'https://liulanqi.baidu.com/',
     ...
     *   - Vivaldi / YaBrowser / UCBrowser / Whale / DuckDuckGo / Silk / Maxthon e as skins
     *     multi-variante (QQ / Baidu / 360) antes de Chrome (todas carregam "Chrome/" no UA).
    ...
    function viaUserAgent() {
        ...
        if ((v = extrair(ua, /\bSilk\/(\d+(?:\.\d+)?)/)))        return { codigo: 'sl',  versao: v };
        if ((v = extrair(ua, /\bMaxthon\/(\d+(?:\.\d+)?)/)))     return { codigo: 'mx',  versao: v };

        // Skins multi-variante (QQ, Baidu, 360): a versão própria não reflete a capacidade
        // (varia entre desktop/mobile ou usa o motor X5). Classificadas pela versão do Chrome
        // embutido. Sem token de Chrome, caem nos ramos seguintes (ex.: 360 sobre Trident → IE).
        if (/\b(?:MQQBrowser|QQBrowser)\//i.test(ua)     && (v = extrair(ua, /\bChrome\/(\d+(?:\.\d+)?)/))) return { codigo: 'qq',  versao: v };
        if (/\b(?:baiduboxapp|baidubrowser)\//i.test(ua) && (v = extrair(ua, /\bChrome\/(\d+(?:\.\d+)?)/))) return { codigo: 'bd',  versao: v };
        if (/\b360(?:SE|EE)\b/i.test(ua)                 && (v = extrair(ua, /\bChrome\/(\d+(?:\.\d+)?)/))) return { codigo: '360', versao: v };
